...
class _MapPageState extends State<MapPage> {
  ...
  // When false, only edges defined in lib/data/waypoints.dart (initialGraphEdges)
  // will be used. Set to true to re-enable automatic nearest-neighbor edges.
  final bool _useAutoGeneratedEdges = false;
  ...
  Future<void> _initializeLocationTracking() async {
    ...
    _positionStreamSubscription = Geolocator.getPositionStream(
      ...
      if (_isNavigating && _currentNavigationTarget != null && _computedPath.isNotEmpty) {
                      ...
                      children: [
                        ...
                        MarkerLayer(
                          markers: [
                            ...
                            if (_showWaypoints)
                              for (final entry in wpdata.initialGraphNodes.entries)
                                Marker(
                                  point: entry.value,
                                  width: 18,
                                  height: 18,
                                  child: Stack(
                                    ...
                                    children: [
                                      ...
                                      Builder(builder: (ctx) {
                                        try {
                                          final idx = wpdata.initialGraphNodeKeys.indexOf(entry.key);
  ...
  List<Marker> _buildMapMarkers() {
    ...
    for (var b in buildingLocations) {
      ...
      try {
        ...
        if (mapped != null && mapped.isNotEmpty && wpdata.initialGraphNodes.containsKey(mapped.first)) {
          final wp = wpdata.initialGraphNodes[mapped.first]!;
          b['lat'] = wp.latitude;
          b['lng'] = wp.longitude;
  ...
  _graphNodes.addAll(wpdata.initialGraphNodes);
  ...
  void _initializeGraphData() {
    ...
    // Merge in any pre-defined waypoint edges from the external data file
    try {
      for (final from in wpdata.initialGraphEdges.keys) {
        _graphEdges.putIfAbsent(from, () => {});
        for (final to in wpdata.initialGraphEdges[from]!.keys) {
          _graphEdges[from]![to] = wpdata.initialGraphEdges[from]![to]!;
        }
      }
    } catch (e) {
      debugPrint('Error merging waypoint edges: $e');
    }

    // Optionally compute neighbor distances and connect each node to a few
    // nearest neighbors. Disabled by default so only manual connections
    // defined in `lib/data/waypoints.dart` (initialGraphEdges) are used.
    if (_useAutoGeneratedEdges) {
